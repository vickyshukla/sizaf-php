...
<section class="bg-gradient-to-br from-slate-50 to-slate-100 py-16 px-4">
    <div class="max-w-7xl mx-auto">
        <div class="grid lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="bg-slate-900 border-0 shadow-xl text-white sticky top-8 rounded-lg">
                    ...
                    <div class="px-6 pb-6 space-y-2">
                        <?php
                        $industries = [
                            ['icon' => 'shield', 'name' => "Governance & Security"],
                            ['icon' => 'heart', 'name' => "Health & Education"],
                            ['icon' => 'building', 'name' => "Non profit organization"],
                            ['icon' => 'plane', 'name' => "Travel & Leisure"],
                            ['icon' => 'construction', 'name' => "Construction & Real Estate"],
                            ['icon' => 'megaphone', 'name' => "Media & Advertising"],
                            ['icon' => 'shopping-cart', 'name' => "Retail & E-Commerce"],
                            ['icon' => 'factory', 'name' => "Production, Engg & Pharma"],
                            ['icon' => 'truck', 'name' => "Transport & Logistics"],
                            ['icon' => 'leaf', 'name' => "Agri, Fisheries & Climate"],
                            ['icon' => 'banknote', 'name' => "Finance & Banking"]
                        ];
                        
                        $selectedIndustry = "Governance & Security"; // Default selected
                        
                        foreach ($industries as $industry): ?>
                            <div 
                                onclick="setSelectedIndustry('<?= $industry['name'] ?>')"
                                class="flex items-center gap-3 p-3 rounded-lg transition-all cursor-pointer <?= $selectedIndustry === $industry['name'] ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white' : 'hover:bg-slate-800 text-slate-300 hover:text-white' ?>"
                            >
                                <svg 
                                    class="h-5 w-5 <?= $selectedIndustry === $industry['name'] ? 'text-white' : 'text-slate-400' ?>" 
                                    fill="none" 
                                    stroke="currentColor" 
                                    viewBox="0 0 24 24" 
                                ...
                                <?php if ($selectedIndustry === $industry['name']): ?>
            ...
            <div class="lg:col-span-3 space-y-8">
                <?php if ($selectedIndustry === "Governance & Security"): ?>
    ...
    <script>
        function setSelectedIndustry(industry) {
            // This would typically be handled by server-side logic in PHP
            // For demo purposes, we'll just reload the page with a parameter
            window.location.href = window.location.pathname + '?industry=' + encodeURIComponent(industry);
        }
    </script>
</section>
</div>
